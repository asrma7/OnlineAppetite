...
//error size
if (sizeof($errors) == 0) {
    $sql = "UPDATE CATEGORIES SET CATEGORY_NAME = '$categoryName', DESCRIPTION = '$description'";
    if (!empty($categoryImage)) {
        $sql .= ", IMAGE = '$categoryImage'";
    }
    $sql .= "WHERE CATEGORY_ID = '$categoryID'";
    $res = query($sql);
    if (!$res)
        $_SESSION['message'] = ["message" => "Error while updating Category", 'color' => "danger"];
    else
        $_SESSION['message'] = ["message" => "Category updated Successfully!", 'color' => "success"];
} else {
    $_SESSION['message'] = ["message" => "Please fix the following errors", 'color' => "danger"];
    $_SESSION['errors'] = $errors;
    $_SESSION['old'] = $old;
}
header('Location:/admin/editCategory.php?id='.$categoryID);
