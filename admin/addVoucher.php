  ...
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      <section class="content">
        <div class="container-fluid">
          <h5 class="mb-2">Add Voucher</h5>

          <form class="addForm" action="insertVoucher.php" method="POST">
            ...
            <div class="form-group">
              ...
              <input type="text" id="voucherCode" name="voucherCode" class="form-control <?= isset($errors['voucherCode']) ? 'is-invalid' : ''; ?>" value="<?= $old['voucherCode'] ?? ''; ?>">
              <?= isset($errors['voucherCode']) ? '<div class="invalid-feedback">' . $errors['voucherCode'] . '</div>' : ''; ?>
            </div>
            <!--discount amount-->
            <div class="form-group">
              <label for="amount">Discount Amount</label>
              <input type="text" id="amount" name="amount" class="form-control <?= isset($errors['amount']) ? 'is-invalid' : ''; ?>" value="<?= $old['amount'] ?? ''; ?>">
              <?= isset($errors['amount']) ? '<div class="invalid-feedback">' . $errors['amount'] . '</div>' : ''; ?>
            </div>
            <!--minimum amount-->
            <div class="form-group">
              <label for="minimum">Minimum Amount</label>
              <input type="text" id="minimum" name="minimum" class="form-control <?= isset($errors['minimum']) ? 'is-invalid' : ''; ?>" value="<?= $old['minimum'] ?? ''; ?>">
              <?= isset($errors['minimum']) ? '<div class="invalid-feedback">' . $errors['minimum'] . '</div>' : ''; ?>
            </div>
            <!--submit button-->
            <button type="submit" class="btn btn-outline-secondary mb-3">Add Voucher</button>
          </form>
