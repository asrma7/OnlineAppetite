...
$fetch = fetch_row("SELECT * FROM SHOPS WHERE SHOP_ID = '$shop_id'");
if (!$fetch) {
  header('Location: ../404.php');
  exit();
...
$fetch['shop_type'] = $old['shop_type'] ?? $fetch['SHOP_TYPE'];
?>
  ...
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      <section class="content">
        <div class="container-fluid">
          <h5 class="mb-2">Edit Shop</h5>

          <form class="addForm" action="updateShop.php" method="POST">
            ...
            <div class="form-group">
              ...
              <input type="text" id="shopName" name="shopName" class="form-control <?= isset($errors['shopName']) ? 'is-invalid' : ''; ?>" value="<?= $old['shopName'] ?? $fetch['SHOP_NAME']; ?>">
              <?= isset($errors['shopName']) ? '<div class="invalid-feedback">' . $errors['shopName'] . '</div>' : ''; ?>
            </div>
            <!--pan no.-->
            <div class="form-group">
              <label for="govNum">Pan no./Vat no.</label>
              <input type="text" id="govNum" name="govNum" class="form-control <?= isset($errors['govNum']) ? 'is-invalid' : ''; ?>" value="<?= $old['govNum'] ?? $fetch['GOV_NO']; ?>">
              <?= isset($errors['govNum']) ? '<div class="invalid-feedback">' . $errors['govNum'] . '</div>' : ''; ?>
            </div>
            <!--address-->
            <div class="form-group">
              <label for="address">Address</label>
              <input type="text" id="address" name="address" class="form-control <?= isset($errors['address']) ? 'is-invalid' : ''; ?>" value="<?= $old['address'] ?? $fetch['ADDRESS']; ?>">
              <?= isset($errors['address']) ? '<div class="invalid-feedback">' . $errors['address'] . '</div>' : ''; ?>
            </div>
            <!--contact number-->
            <div class="form-group">
              <label for="contact">Contact Number</label>
              <input type="text" id="contact" name="contact" class="form-control <?= isset($errors['contact']) ? 'is-invalid' : ''; ?>" value="<?= $old['contact'] ?? $fetch['CONTACT_NO']; ?>">
            ...
            <div class="form-group">
              ...
              <select class="form-control select2 select2-danger <?= isset($errors['shop_type']) ? 'is-invalid' : ''; ?>" id="shop_type" name="shop_type" data-dropdown-css-class="select2-danger" style="width: 100%;">
                <option <?= $fetch['shop_type'] == 'small' ? 'selected' : ''; ?> value="small">Small</option>
                <option <?= $fetch['shop_type'] == 'medium' ? 'selected' : ''; ?> value="medium">Medium</option>
                <option <?= $fetch['shop_type'] == 'large' ? 'selected' : ''; ?> value="large">Large</option>
              </select>
            ...
            <!--submit button-->
            <button type="submit" class="btn btn-outline-secondary mb-3">Edit Shop</button>
          </form>
