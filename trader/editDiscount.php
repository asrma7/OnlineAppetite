...
if (!$fetch) {
  header('Location: ../404.php');
  exit();
}elseif ($fetch['CREATED_BY']!=$_SESSION['trader']['USER_ID']) {
  header('Location: ../401.php');
  exit();
}
$fetch['target'] = $old['target'] ?? $fetch['TARGET_ID'];
?>
          ...
          <form class="addForm" action="updateDiscount.php" method="POST">
            ...
            <div class="form-group">
              ...
              <select class="form-control select2 select2-danger <?= isset($errors['target']) ? 'is-invalid' : ''; ?>" id="target" name="target" data-dropdown-css-class="select2-danger" style="width: 100%;">
                ...
                ?>
                  <option <?= $fetch['target'] == $product['PRODUCT_ID'] ? 'selected' : ''; ?> value="<?= $product['PRODUCT_ID'] ?>"><?= $product['PRODUCT_NAME'] ?></option>
                <?php } ?>
              </select>
