...
<body class="hold-transition sidebar-mini">
  <div class="wrapper">
    ...
    $products = fetch_all_row("SELECT PRODUCTS.*, (SELECT CATEGORY_NAME FROM CATEGORIES WHERE CATEGORIES.CATEGORY_ID = PRODUCTS.CATEGORY_ID) AS CATEGORY_NAME FROM SHOPS INNER JOIN PRODUCTS ON PRODUCTS.SHOP_ID = SHOPS.SHOP_ID WHERE PRODUCTS.TRADER_ID = '$user_id'");
    ?>
            ...
            <tbody>
              <?php foreach ($products as $product) { ?>
                <tr>
                  ...
                  <td>
                    <div class="d-flex">
                      <button class="btn btn-warning m-1" onclick="window.location.href='editProduct.php?id=<?= $product['PRODUCT_ID'] ?>'">Edit</button>
                      <button class="btn btn-danger m-1" onclick="window.location.replace('deleteProduct.php?id=<?= $product['PRODUCT_ID'] ?>')">Delete</button>
                    </div>
                  </td>
                </tr>
              <?php } ?>
