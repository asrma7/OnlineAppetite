...
$fetch = fetch_row("SELECT * FROM PRODUCTS WHERE PRODUCT_ID = '$product_id'");
if (!$fetch) {
  header('Location: ../404.php');
  exit();
}elseif ($fetch['TRADER_ID']!=$_SESSION['trader']['USER_ID']) {
  header('Location: ../401.php');
  exit();
}
$fetch['category'] = $old['category'] ?? $fetch['CATEGORY_ID'];
$fetch['shop'] = $old['shop'] ?? $fetch['SHOP_ID'];
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="content-wrapper">
      ...
      <section class="content">
        <div class="container-fluid">
          ...
          <form class="addForm" action="updateProduct.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <input type="text" id="productName" name="productName" class="form-control <?= isset($errors['productName']) ? 'is-invalid' : ''; ?>" value="<?= $old['productName'] ?? $fetch['PRODUCT_NAME']; ?>">
              <?= isset($errors['productName']) ? '<div class="invalid-feedback">' . $errors['productName'] . '</div>' : ''; ?>
            </div>
            <!--price-->
            <div class="form-group">
              <label for="price">Price</label>
              <input type="text" id="price" name="price" class="form-control <?= isset($errors['price']) ? 'is-invalid' : ''; ?>" value="<?= $old['price'] ?? $fetch['PRICE']; ?>">
            ...
            <div class="form-group">
              ...
              <select class="form-control select2 select2-danger <?= isset($errors['category']) ? 'is-invalid' : ''; ?>" id="category" name="category" data-dropdown-css-class="select2-danger" style="width: 100%;">
                ...
                ?>
                  <option <?= $fetch['category'] == $category['CATEGORY_ID'] ? 'selected' : ''; ?> value="<?= $category['CATEGORY_ID'] ?>"><?= $category['CATEGORY_NAME'] ?></option>
                <?php } ?>
              </select>
            ...
            <div class="form-group">
              ...
              <input type="number" id="stock" name="stock" class="form-control <?= isset($errors['stock']) ? 'is-invalid' : ''; ?>" value="<?= $old['stock'] ?? $fetch['STOCK']; ?>">
            ...
            <div class="form-group">
              ...
              <select class="form-control select2 select2-danger <?= isset($errors['shop']) ? 'is-invalid' : ''; ?>" id="shop" name="shop" data-dropdown-css-class="select2-danger" style="width: 100%;">
                ...
                ?>
                  <option <?= $fetch['shop'] == $shop['SHOP_ID'] ? 'selected' : ''; ?> value="<?= $shop['SHOP_ID'] ?>"><?= $shop['SHOP_NAME'] ?></option>
                <?php } ?>
              </select>
            ...
            <div class="form-group">
              ...
              <textarea id="inputDescription" class="form-control <?= isset($errors['description']) ? 'is-invalid' : ''; ?>" name="description" rows="4"><?= $old['description'] ?? $fetch['DESCRIPTION']; ?></textarea>
            ...
            <!--submit button-->
            <button type="submit" class="btn btn-outline-secondary mb-3">Edit Product</button>
          </form>
